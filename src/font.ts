export async function loadGoogleFont({
  ...
  // Cache font data as well
  let fontRes = await cache.match(fontUrl);
  if (!fontRes) {
    fontRes = await fetch(fontUrl);
    fontRes = new Response(fontRes.body, fontRes);
    fontRes.headers.set("Cache-Control", "public, max-age=31536000, immutable");
    await cache.put(fontUrl, fontRes.clone());
  }
